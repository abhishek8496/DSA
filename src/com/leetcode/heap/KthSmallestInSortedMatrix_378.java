...
class Elements
{
    ...
    int row;
    int col;
    Elements(int val,int row,int col)
    {
        this.val=val;
        this.row=row;
        this.col=col;
...
public class KthSmallestInSortedMatrix_378 {
	    ...
	    public static int kthSmallest(int[][] matrix, int k) {
	        ...
	        PriorityQueue<Elements> minHeap = new PriorityQueue<Elements>((e1,e2)->e1.val-e2.val);
	        for(int i = 0 ; i <matrix.length ; i++)
	        {
	            minHeap.add(new Elements(matrix[i][0],i,0));
	        }
	        ...
	        while(counter!=k-1)
	        {
	            Elements ele = minHeap.poll();
	            if(ele.col+1<matrix.length)
	            {
	                minHeap.add(new Elements(matrix[ele.row][ele.col+1],ele.row,ele.col+1));
	            }
	            counter++;
	        }
